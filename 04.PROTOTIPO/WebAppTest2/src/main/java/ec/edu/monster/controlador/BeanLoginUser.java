...
import ec.edu.monster.dao.DAORegistroAcceso;
...
*/
import ec.edu.monster.config.util.AES256;
import ec.edu.monster.config.util.FechaUtiles;
import ec.edu.monster.config.util.Utilidades;
import ec.edu.monster.config.util.Validaciones;
...
import ec.edu.monster.config.util.LogUtiles;
import ec.edu.monster.config.util.PrimeUtiles;
import ec.edu.monster.config.util.SOUtiles;
...
@ManagedBean
@ViewScoped
public class BeanLoginUser extends FiltroAcceso implements Serializable {
    ...
    private final String navegadorUsuario;
    ...
    private static final Logger LOG = Logger.getLogger(BeanLoginUser.class.getName());
    ...
    public BeanLoginUser() {
        ...
        navegadorUsuario = SOUtiles.navegador();
        /*
        daoUsuarioAcceso = new DAOUsuarioAcceso();

        configClave = new ConfigClave();*/

        LOG.log(Level.INFO, "[fecha:{0}] Se ha accedido al ambiente {1} con el navegador:{2} desde la ip: {3}", new Object[]{FechaUtiles.ahora(), url, navegadorUsuario, ipIngreso});
    }
    ...
    public void validarPerfil() throws Exception {
        ...
        LOG.log(Level.INFO, "Perfil seleccionado {0}", "un perfil reemplazar");
        validarUsuario();
    }
    ...
    private void seguridadNivel1(String tipoLoggeo, String nombrePerfil, UsuarioAcceso usuario) throws Exception {
        ...
        if (usuario.getUsuario().getUsrEstatus().equals(EstadosConfig.BLOQUEADO.getCodigo())) {
            ...
            List<UsuarioClave> contrasenias = new DAOContrasenia().obtenerUltimasContraseniasPorUsuarioAcceso(usuario.getIdUsuarioAcceso(), 1);
            if (contrasenias != null && contrasenias.size() > 0) {
                ultimaContrasenia = contrasenias.get(0);
                if (AES256.verificarHash(claveUsuario, ultimaContrasenia.getUsrClaveContenido())) {
                    ...
                    if (Validaciones.obtenerDiasDesdeFechaIngresadaHastaFechaActual(ultimaContrasenia.getUsrClaveFecha()) >= configClave.getClaveTiempoCaducar()) {
                        ...
                        if (asignaciones != null && asignaciones.size() > 0) {
                            ...
                            if (asignaciones.size() > 1 && perfilSeleccionado == null) {
                                ...
                                if (perfil.getPerfilEstatus2().equals(EstadosConfig.PERFIL_REPORTERIA.getCodigo())) {
                                    ...
                                    PrimeUtiles.mostrarMensaje(FacesMessage.SEVERITY_ERROR, "Su perfil NO es de REPORTERIA.");
                                    tipoLoggeo = EstadosConfig.LOGIN_DESACTIVADO.getCodigo();
                                    perfilSeleccionado = null;
            ...
            if (intFallidos >= configClave.getIntentosFallidos()) {
                ...
                try {
                    ...
                    if (DAOSaveGenerico.guardarLotes(listaObjetos)) {
                        PrimeUtiles.mostrarMensaje(FacesMessage.SEVERITY_INFO, "Ha excedido el numero maximo de intentos su Usuario ha sido BLOQUEADO. Por favor cominiquese con el Admnistrador del sistema.");
                        ...
                        LOG.log(Level.INFO, "Por favor revise su conexión a internet e intente de nuevo.");
